...
if(filter_input(INPUT_POST,'add_to_cart')){
	if(isset($_SESSION['shopping_cart'])){
		...
		//create sequantial array for matching arrat keys to product ids
		...
		if(!in_array(filter_input(INPUT_GET,'food_id'),$product_ids)){
			...
			//match array key to food_id of the product being added to the cart
		...
		//creating array using submitted form data, start from key 0 and fill it eith values
...
if(filter_input(INPUT_GET,'action')=='delete'){
	//loop through all the products in the shopping cart until it matches with the GET food_id variable
	foreach($_SESSION['shopping_cart'] as $key=>$product){
		if($product['food_id']==filter_input(INPUT_GET,'food_id')){
			unset($_SESSION['shopping_cart'][$key]);
		}
	}
	//reset the session array keys so that they match with $product_ids numeric array
...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
	...
	<?php
		...
		$connect=mysqli_connect('localhost','root','join','project');
			...
			if(mysqli_num_rows($result)>0):
				...
				<div class="col-sm-4 col-md-3">
					<form method="post" action="cart.php?action=add&food_id=<?php echo $product['food_id'];?>">
						<div class="products">
							<img src="<?php echo $product['image']; ?>" class="img-responsive" class="cards" style="height: 150px; width:150px;"/>
							<h4 class="text-info"><?php echo $product['food']; ?></h4>
							<h4 class="text"><?php echo $product['description'];?></h4>
		...
		<tr>
			...
			<td>
				<a href="cart.php?action=delete&food_id=<?php echo $product['food_id'];?>">
